...
public class RequestManager extends Service {
	...
	private void processRequests() {
		for (Client client : server.getClients()) {
			if (client.hasData()) {
				Job processorJob = getProcessorInstance(client, client.receive());
				if (processorJob != null) workers.assign(processorJob);
			}
		}
	}

	private RequestProcessorJob getProcessorInstance(Client requestor, Object request) {
		try {
			return requestProcessor.getDeclaredConstructor(Client.class, Object.class).newInstance(requestor, request);
