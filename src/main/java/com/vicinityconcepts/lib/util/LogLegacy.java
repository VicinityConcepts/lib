...
public final class LogLegacy {
	...
	public static void stop() throws IOException {
		...
		try {
			...
			LogLegacy.error(ERROR_CLOSE_INTERRUPTED);
		} finally {
			running = false;
	...
	private static void log(Level level, String message, Throwable t) {
		try {
			LOGGER.log(new InternalLogEntry(level, message, Caller.infer(LogLegacy.class.getName()), t));
